...
function startGame() {
  ...
  const statusText = document.getElementById('header-status-text');
  if (statusText) {
    statusText.classList.remove('timer-fever-warn');
  }
...
function handlePlayerKeyInput(playerId, key) {
  ...
  if (player.currentInput.length === q.answerStr.length) {
    ...
    if (isCorrect) {
      ...
      let addedScore = Math.round(baseScore * multiplierBonus);
      
      // 마지막 10초 피버타임 적용 시 점수 2배 가산
      ...
      if (player.combo > 0) {
        let comboText = `${player.combo} COMBO`;
        if (isFeverTimeActive) {
          comboText += ` (FEVER x2!)`;
        }
        comboBadge.innerText = comboText;
...
function gameLoopTick() {
  ...
  if (gameMode === 'survival') {
    ...
    if (gameTimeLeft <= 0) {
      ...
      // 마지막 10초전 피버타임 작동 돌입
      if (gameTimeLeft <= 10.0 && !isFeverTimeActive) {
        triggerFeverTimeStart();
      }
...
function triggerFeverTimeStart() {
  ...
  const statusText = document.getElementById('header-status-text');
  if (statusText) {
    statusText.classList.add('timer-fever-warn');
  }
  
  // 3) 모든 플레이어 칼럼에 피버타임 핫핑크 네온 펄스 클래스 적용
  players.forEach(player => {
    const colEl = document.getElementById(`player-col-${player.id}`);
    if (colEl) {
      colEl.classList.add('fever-time-active');
    }
  });
  
  // 4) 화면 전체 중앙에 피버타임 시작 팝업 오버레이 동적 생성 후 1.4초 뒤 자동 페이드아웃 및 제거
  const appContainer = document.getElementById('app-container');
  if (appContainer) {
    const toast = document.createElement('div');
    toast.className = 'fever-start-toast';
    toast.innerHTML = `
      <h2>🔥 FEVER TIME 🔥</h2>
      <p>마지막 10초! 점수 2배 역전 찬스!</p>
    `;
